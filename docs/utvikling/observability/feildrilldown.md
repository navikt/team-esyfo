...
Overgangstilstander står synlig i runtime-velgeren. Det gjør at `esyfovarsel` kan følges under migreringen til `syfo-budstikka`. `syfobrukertilgang` er avviklet og er ikke del av den operative velgeren; uverifiserte standalone PrometheusRule-instanser følges i alert-registeret.
