...
Produksjonsflåten genereres fra [runtimeinventaret](./runtimeinventar): 25 operative GCP-appkomponenter, hvorav 23 har HTTP/SERVER-profil. Avviklede `syfobrukertilgang` og `syfooppfolgingsplanservice` er ikke med. `esyfovarsel` og `syfo-budstikka` er workers og skal ikke vurderes med HTTP-måledekning.
