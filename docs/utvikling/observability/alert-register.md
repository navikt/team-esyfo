...
`RETIRE` betyr heller ikke «slett nå». Hver pensjonering har enten verifisert/begrunnet bortfall eller en eksplisitt gate. `syfobrukertilgang`-runtime ble slettet etter V5-cutoveren, men de tre `prod-gcp`- og tre foreldreløse `prod-fss`-instansene beholdes som uverifisert oppryddingsgjeld til live-avstemming beviser at de er borte; arbeidet følges i [#369](https://github.com/navikt/syfobrukertilgang/issues/369) og [#368](https://github.com/navikt/syfobrukertilgang/issues/368).
...
- Tre `syfobrukertilgang`-regler var fortsatt enabled/inaktive i `prod-fss` ved snapshotet, mens [NAIS-applikasjonslisten](https://console.nav.cloud.nais.io/team/team-esyfo/applications) da bare viste tjenesten i `dev-gcp` og `prod-gcp`. [Commit `9c9a259c7926`](https://github.com/navikt/syfobrukertilgang/commit/9c9a259c7926093335a24788ed9fd82d00406d82) flyttet alert-workflowen fra `prod-fss` til `prod-gcp` 22. mai 2023; den slettet ikke alertfilen. [Commit `22b66f6950f8`](https://github.com/navikt/syfobrukertilgang/blob/22b66f6950f874ac10dd5c2012c67c7a0835154e/alerts.yaml) er siste repository-snapshot før denne cluster-cutoveren. Den er **ikke** bevis på hvilken SHA som sist ble deployert til FSS. Runtime ble slettet 18. september 2026, men de tre `prod-gcp`- og tre `prod-fss`-instansene er fortsatt uverifisert oppryddingsgjeld i [#369](https://github.com/navikt/syfobrukertilgang/issues/369) og [#368](https://github.com/navikt/syfobrukertilgang/issues/368).
...
- Regler for `syfobrukertilgang` beholdes som uverifisert oppryddingsgjeld etter at runtime ble slettet 18. september 2026. Tre deployerte `prod-gcp`- og tre `prod-fss`-instanser skal først fjernes når live-avstemming beviser at de er borte; dette følges i [#369](https://github.com/navikt/syfobrukertilgang/issues/369) og [#368](https://github.com/navikt/syfobrukertilgang/issues/368).
- De tre legacyreglene for `syfooppfolgingsplanservice` beholdes som oppryddingsgjeld etter bekreftet tjenestestopp 2. september 2026. De fjernes først når en ny live-avstemming har bevist at restreglene er borte; dette følges i [#208-guiden](./avvikling-syfooppfolgingsplanservice).
