...
En Kafka-pipeline får alltid det maskinelle gapet `pipeline-contract` så lenge kontrakten står som `proposed`. Ingen av topicene har ennå en aktivert og evaluert service level. Derfor står de maskinelle topic-fristene fortsatt `IKKE DEFINERT`. Nulltrafikkpolicy og consumer-lag brukes heller ikke til å evaluere produksjonshelse før den operative topic-kontrakten er godkjent i #212. Dermed kan selv perfekte tekniske signaler ikke gjøre en uavklart pipeline grønn.
...
I arbeidet med [#205](https://github.com/navikt/team-esyfo/issues/205) ble 23 av de 26 daværende runtimene funnet i NAIS APM-katalogen for `team-esyfo/prod`, og disse har tidsstemplet `linked`-evidens. `aktivitetskrav-frontend`, `dialogmote-microfrontend` og `lumi-dashboard` står fortsatt som `unverified`. En fungerende tjenesterute beviser ikke i seg selv komplett trace- eller metrikksdekning.

`syfobrukertilgang` ble markert `retired` 18. september 2026 da NAIS-runtime ble slettet etter `syfomotebehov` V5-cutoveren. V5 bruker Dine sykmeldtes TokenX-bundne nærmeste-leder-relasjon, ikke `esyfo-narmesteleder` direkte. De seks standalone PrometheusRule-instansene er fortsatt uverifisert oppryddingsgjeld i [#368](https://github.com/navikt/syfobrukertilgang/issues/368) og [#369](https://github.com/navikt/syfobrukertilgang/issues/369), og er ikke del av operativ runtime. `syfooppfolgingsplanservice`, Redis og exporter ble markert `retired` etter bekreftet tjenestestopp 2. september 2026; eventuelle observerte legacyressurser er nå drift som følges i [#208](https://github.com/navikt/team-esyfo/issues/208).
