...
I arbeidet med [#205](https://github.com/navikt/team-esyfo/issues/205) ble 23 av de 25 daværende runtimene funnet i NAIS APM-katalogen for `team-esyfo/prod`, og disse har tidsstemplet `linked`-evidens. `aktivitetskrav-frontend` og `dialogmote-microfrontend` står fortsatt som `unverified`. En fungerende tjenesterute beviser ikke i seg selv komplett trace- eller metrikksdekning.

`syfooppfolgingsplanservice`, Redis og exporter ble markert `retired` etter bekreftet tjenestestopp 2. september 2026; eventuelle observerte legacyressurser er nå drift som følges i [#208](https://github.com/navikt/team-esyfo/issues/208).
